...
const RentCustomersHistory = () => {
  const {customer_id} = useParams()
  const [receipts, setReceipts] = useState([])

  const fetchHistory = async () => {
    try {
      const { data } = await axios.get(`/rent/history/${customer_id}`);
  ...
  return (
    <DashboardLayout>
      <h3 className='font-medium text-center m-4' >Tarix</h3>
...
export default RentCustomersHistory